...
| Ficheiro | Descrição |
|----------|-----------|
| `EXECUTAR.bat` | O *Launcher* principal. Verifica privilégios de Admin, testa a internet e instala automaticamente o Python, o Chocolatey e as dependências (PIP) antes de chamar a interface. |
| `gui.py` | Interface gráfica moderna desenvolvida em `customtkinter`, permitindo a seleção granular das políticas a aplicar. |
| `mod_config.py` | O "Cérebro" de configuração. Injeta chaves de registo (*Registry*), gere o *Firewall*, limpa os *widgets* e implementa o Watchdog de Auto-Cura. |
| `mod_instalar.py` | Motor de *deploy*. Comunica com o Chocolatey e PowerShell para baixar softwares e drivers. |
| `settings.json` | Ficheiro de dicionário. Define quais softwares serão instalados e os caminhos de imagens corporativas (Logos/Wallpapers). |
...
* **Conectividade:** Ligação à Internet ativa (necessária para baixar dependências, softwares e atualizar o relógio via NTP.br).
...
8. No final, será gerado um relatório de *Hardware Snapshot* na pasta `C:\Scripts\`.
...
Este script realiza alterações profundas no Registo do Windows (*Registry*), cria tarefas agendadas e remove pacotes nativos do sistema operativo. Embora amplamente testado em ambientes de produção, o uso é da inteira responsabilidade do utilizador. Recomenda-se testar em ambiente controlado ou máquina virtual antes de aplicar numa infraestrutura em grande escala.
