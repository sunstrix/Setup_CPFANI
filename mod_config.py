"""mod_config.py — V5.9.5.4 (Edição CP Fani: Desativação do PrtSc Nativo, GPO OneDrive, Liberação Win+G e Reset de Cache)"""
import winreg
import subprocess
import os
import ctypes
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        subprocess.run(cmd_keyboard, capture_output=True, creationflags=0x08000000)
                        ...
                        subprocess.run(cmd_sync, capture_output=True, creationflags=0x08000000)
                        
                        # Remove ganchos secundários de ferramentas de nuvem no perfil
...
"""
    ...
    subprocess.run(f'schtasks /create /tn "CPFANI_Watchdog" /tr "wscript.exe \\"{vbs_path}\\"" /sc onlogon /ru "SYSTEM" /rl highest /f', shell=True, capture_output=True, creationflags=0x08000000)
...
def sync_time_ntp():
    try:
        cmds = ['w32tm /config /manualpeerlist:"a.ntp.br b.ntp.br c.ntp.br" /syncfromflags:manual /reliable:YES /update', 'net stop w32time', 'net start w32time', 'w32tm /resync /force']
        for cmd in cmds: subprocess.run(cmd, shell=True, capture_output=True, creationflags=0x08000000)
...
def schedule_daily_reboot():
    try:
        ...
        subprocess.run(f'schtasks /create /tn "CPFANI_ReinicioDiario" /tr "{task_cmd}" /sc daily /st 21:00 /ru "SYSTEM" /rl highest /f', shell=True, capture_output=True, creationflags=0x08000000)
    except: pass
...
def set_apps_to_startup_all_users():
    ...
    _log("Nivel 11: Sanando cache do Explorer, OneDrive e aplicando bloqueio do painel...", "INFO")
    try:
        # Finaliza processos concorrentes para liberar escrita limpa no registro
        ...
        subprocess.run(["taskkill", "/f", "/im", "sharex.exe"], capture_output=True, creationflags=0x08000000)
        ...
        # 1. Configura as GPOs administrativas locais para bloquear interceptações de nuvem
        set_reg(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\OneDrive", "DisableScreenshotCapture", 1, winreg.REG_DWORD)
        set_reg(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\TabletPC", "DisableSnippingTool", 1, winreg.REG_DWORD)
        
        # 🌟 COMPONENTE ADICIONAL V5.9.5.4: Desativa a Xbox Game Bar para libertar o atalho Win + G globalmente
        ...
        if sid:
            subprocess.run(["reg", "add", f"HKU\\{sid}\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\GameDVR", "/v", "AppCaptureEnabled", "/t", "REG_DWORD", "/d", "0", "/f"], capture_output=True, creationflags=0x08000000)
            subprocess.run(["reg", "add", f"HKU\\{sid}\\System\\GameConfigStore", "/v", "GameDVR_Enabled", "/t", "REG_DWORD", "/d", "0", "/f"], capture_output=True, creationflags=0x08000000)
    ...
    for link, paths in apps.items():
        exe_found = None
        for p in paths:
            if os.path.exists(p): exe_found = p; break
        if exe_found:
            target = os.path.join(startup_path, link)
            subprocess.run(f'powershell "$s=(New-Object -COM WScript.Shell).CreateShortcut(\'{target}\');$s.TargetPath=\'{exe_found}\';$s.Save()"', shell=True, capture_output=True, creationflags=0x08000000)
...
def remove_agressive_bloatware(bloatware_list):
    for app in bloatware_list:
        try:
            ...
            subprocess.run(["powershell", "-NoProfile", "-Command", f"{cmd_user}; {cmd_prov}"], capture_output=True, text=True, timeout=15, creationflags=0x08000000)
        except: pass
    return True
...
def apply_cpfani_branding(bar_alignment):
    ...
    if bar_alignment != "nenhum":
        ...
        try:
            sid = _get_active_user_sid()
            if sid: subprocess.run(["reg", "add", f"HKU\\{sid}\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced", "/v", "TaskbarAl", "/t", "REG_DWORD", "/d", str(val), "/f"], capture_output=True, creationflags=0x08000000)
        except: pass
    apply_default_user_profile(bar_alignment)

def apply_security_lgpd(apply_lgpd=True, disable_hello=True):
    _log("Aplicando politicas de Securitárias e LGPD...", "INFO")
    sid = _get_active_user_sid()
    if apply_lgpd:
        set_reg(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\DataCollection", "AllowTelemetry", 0, winreg.REG_DWORD)
        if sid: subprocess.run(["reg", "add", f"HKU\\{sid}\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager", "/v", "SubscribedContent-338389Enabled", "/t", "REG_DWORD", "/d", "0", "/f"], capture_output=True, creationflags=0x08000000)
    ...
    if disable_hello: disable_windows_hello_redundant()
...
def _get_image_path(local_path, urls, temp_name):
    ...
    for url in urls:
        try:
            public_temp = r"C:\Users\Public\Downloads"; os.makedirs(public_temp, exist_ok=True)
            temp_path = os.path.join(public_temp, temp_name); urllib.request.urlretrieve(url, temp_path)
            if os.path.exists(temp_path) and os.path.getsize(temp_path) > 10000: return temp_path
        except: pass
    return None

def apply_cpfani_wallpaper_redundant():
    script_dir = os.path.dirname(os.path.abspath(__file__)); local_wp = os.path.join(script_dir, "resources", "wallpaper_cpfani.jpg")
    urls = ["https://drive.google.com/uc?export=download&id=1K5SWWC1dJL0qETRKAVdJtc8-Wi39G83G", "https://github.com/sunstrix/Setup_CPFANI/raw/main/resources/wallpaper_cpfani.jpg"]
    target_path = _get_image_path(local_wp, urls, "cpfani_wp.png")
    if not target_path: return False
    try: ctypes.windll.user32.SystemParametersInfoW(20, 0, target_path, 3)
    except: pass
...
def apply_cpfani_lockscreen_redundant():
    ...
    if not target_path: return False
    set_reg(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\Personalization", "LockScreenImage", target_path, winreg.REG_SZ)
...
def remove_widgets_taskbar():
    try:
        ...
        sid = _get_active_user_sid()
        if sid: subprocess.run(["reg", "add", f"HKU\\{sid}\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced", "/v", "TaskbarDa", "/t", "REG_DWORD", "/d", "0", "/f"], capture_output=True, creationflags=0x08000000)
    except: pass
...
def generate_full_snapshot():
    hw = _get_hardware_info(); pc_name = hw.get("Nome_Computador", "UNKNOWN")
    log_path = Path(f"C:/Scripts/CPFANI_Hardware_Snapshot_{pc_name}.txt"); log_path.parent.mkdir(parents=True, exist_ok=True)
