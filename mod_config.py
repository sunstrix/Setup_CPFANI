# -*- coding: utf-8 -*-
"""mod_config.py - V6.0.2 (Edicao CP Fani: Autenticacao Google Drive Robusta + Pre-checagem)"""
import winreg
import subprocess
import os
import ctypes
...
# ============================================================================
# NOVAS FUNÇÕES DE AUTENTICAÇÃO GOOGLE DRIVE ROBUSTA
# ============================================================================

def _get_token_path():
    """Retorna caminho do token.pickle armazenado por máquina (não por repositório)"""
    credentials_dir = os.path.join(SCRIPT_DIR, "credentials")
    os.makedirs(credentials_dir, exist_ok=True)
    return os.path.join(credentials_dir, "token.pickle")

def _get_credentials_path():
    """Retorna caminho das credenciais OAuth2 (compartilhado entre máquinas)"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(script_dir, "credentials", "oauth2_credentials.json")

def authenticate_google_drive(timeout_seconds=120, callback=None):
    """
    Autentica no Google Drive com validação robusta e timeout.
    
    Args:
        timeout_seconds: Timeout em segundos para o login interativo (padrão: 120s)
        callback: Função opcional callback(status_message) para feedback na GUI
        
    Returns:
        tuple: (service, error_message, user_email)
            - service: Objeto service do Google Drive API se autenticado, None caso contrário
            - error_message: String com mensagem de erro se falhou, None se sucesso
            - user_email: Email da conta autenticada se disponível, None caso contrário
    """
    try:
        ...
        if callback:
            callback(f"Verificando credenciais OAuth2...")
        ...
        # Carregar token existente com validação robusta
        if os.path.exists(token_path):
            try:
                with open(token_path, 'rb') as token:
                    loaded_creds = pickle.load(token)
                    
                    # Validação de tipo: verificar se é instância válida de Credentials
                    if not isinstance(loaded_creds, Credentials):
                        ...
                        try:
                            ...
                            _log(f"[OK] token.pickle corrompido removido: {token_path}", "OK")
                        except Exception as e:
                            _log(f"[ERRO] Falha ao remover token.pickle corrompido: {e}", "ERRO")
                        creds = None
                    else:
                        creds = loaded_creds
                        _log(f"[OK] token.pickle carregado com sucesso", "OK")
                ...
                try:
                    ...
                    _log(f"[ERRO] Falha ao remover token.pickle corrompido: {e2}", "ERRO")
                creds = None
        
        # Validar escopos do token
        ...
        # Renovar token se expirado com try/except amplo
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                try:
                    if callback:
                        callback("Renovando token OAuth2...")
        ...
        # Login interativo com timeout se necessário
        if not creds or not creds.valid:
            try:
                if callback:
                    callback("Aguardando login no navegador... Verifique se uma janela foi aberta.")
                
                flow = InstalledAppFlow.from_client_secrets_file(credentials_path, SCOPES)
                
                # Implementar timeout via threading
                result = [None]
                exception = [None]
                ...
                if thread.is_alive():
                    # Timeout ocorreu
                    error_msg = f"Timeout de {timeout_seconds}s no login interativo. Operacao cancelada."
                    _log(f"[ERRO] {error_msg}", "ERRO")
                    return None, error_msg, None
                
                if exception[0]:
                    error_msg = f"Erro durante login interativo: {exception[0]}"
                    _log(f"[ERRO] {error_msg}", "ERRO")
                    return None, error_msg, None
                ...
                error_msg = f"Erro durante autenticacao OAuth2: {e}"
                _log(f"[ERRO] {error_msg}", "ERRO")
        ...
        try:
            service_temp = build('drive', 'v3', credentials=creds)
            about = service_temp.about().get(fields="user").execute()
            user_info = about.get("user", {})
            user_email = user_info.get("emailAddress", None)
            if user_email:
                _log(f"[OK] Conta Google conectada: {user_email}", "OK")
                if callback:
                    callback(f"✓ Conta Google conectada: {user_email}")
        except Exception as e:
            _log(f"[AVISO] Nao foi possivel obter email do usuario: {e}", "AVISO")
            user_email = None
        
        # Build service final
        service = build('drive', 'v3', credentials=creds)
        return service, None, user_email
        ...
        error_msg = f"Erro inesperado na autenticacao: {e}"
        _log(f"[ERRO] {error_msg}", "ERRO")
        _log(traceback.format_exc(), "ERRO")
        return None, error_msg, None
...
def pre_check_google_drive_auth(timeout_seconds=120, callback=None):
    """
    Pré-checagem de autenticação Google Drive antes do deploy começar.
    
    Args:
        timeout_seconds: Timeout em segundos para login interativo
        callback: Função opcional callback(status_message) para feedback na GUI
        
    Returns:
        tuple: (success, error_message, user_email)
            - success: True se autenticado com sucesso, False caso contrário
            - error_message: String com mensagem de erro se falhou, None se sucesso
            - user_email: Email da conta autenticada se disponível
    """
    _log("=" * 60, "INFO")
    _log("PRE-CHECAGEM DE AUTENTICACAO GOOGLE DRIVE", "INFO")
    _log("=" * 60, "INFO")
    
    service, error_msg, user_email = authenticate_google_drive(
    ...
    if service:
        ...
        try:
            ...
            return True, None, user_email
        except Exception as e:
            error_msg = f"Erro ao acessar pasta do Drive: {e}"
            _log(f"[ERRO] {error_msg}", "ERRO")
            return False, error_msg, user_email
    else:
        return False, error_msg, user_email
...
def generate_full_snapshot(local=None, usuario=None):
    ...
    Returns:
        tuple: (local_path, upload_success, upload_error)
            - local_path: caminho do arquivo local ou None em caso de erro
            - upload_success: True se upload para Drive foi bem-sucedido, False caso contrário
            - upload_error: mensagem de erro se upload falhou, None se sucesso
    """
...
"""
    try:
        ...
        return None, False, f"Erro ao gerar snapshot local: {e}"
    
    # Upload para Google Drive usando nova função de autenticação
    try:
        from googleapiclient.http import MediaFileUpload
        from googleapiclient.errors import HttpError
        
        # Autenticar usando função robusta
        service, auth_error, user_email = authenticate_google_drive()
        
        if not service:
            error_msg = f"Snapshot gerado localmente mas NAO enviado ao Drive. Motivo: {auth_error}. Arquivo salvo em: {local_path}"
            _log(f"[AVISO] {error_msg}", "AVISO")
            return str(local_path), False, error_msg
        ...
        try:
            ...
            error_msg = f"Snapshot gerado localmente mas NAO enviado ao Drive. Motivo: Erro ao acessar pasta do Drive: {e}. Arquivo salvo em: {local_path}"
            _log(f"[ERRO] {error_msg}", "ERRO")
            return str(local_path), False, error_msg
        ...
        error_msg = f"Bibliotecas do Google Drive nao instaladas: {e}. Snapshot salvo apenas localmente em: {local_path}"
        _log(f"[ERRO] {error_msg}", "ERRO")
        _log("Instale com: python -m pip install google-api-python-client google-auth-oauthlib google-auth-httplib2", "INFO")
        return str(local_path), False, error_msg
        
    except HttpError as e:
        error_msg = f"Erro na API do Google Drive: {e}. Snapshot salvo apenas localmente em: {local_path}"
        _log(f"[ERRO] {error_msg}", "ERRO")
        _log(traceback.format_exc(), "ERRO")
        return str(local_path), False, error_msg
        
    except Exception as e:
        error_msg = f"Erro inesperado ao enviar para o Google Drive: {e}. Snapshot salvo apenas localmente em: {local_path}"
        _log(f"[ERRO] {error_msg}", "ERRO")
        _log(traceback.format_exc(), "ERRO")
        return str(local_path), False, error_msg
