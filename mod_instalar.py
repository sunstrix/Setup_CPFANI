...
def _install_rustdesk(timeout=300):
    """Instala RustDesk com fallback para múltiplos métodos"""
    _log("A instalar RustDesk com redundância...", "INFO")
    ...
    try:
        ...
        if res and res.returncode in (0, 1641, 3010, 1638):
            _log("✓ RustDesk instalado via Chocolatey", "OK")
            return True
        else:
            _log(f"Chocolatey retornou código {res.returncode if res else 'None'}", "AVISO")
    except Exception as e:
    ...
    try:
        ...
        if res and res.returncode == 0:
            _log("✓ RustDesk instalado via WinGet", "OK")
            return True
        else:
            _log(f"WinGet retornou código {res.returncode if res else 'None'}", "AVISO")
    except Exception as e:
    ...
    # Tentativa 3: Download direto (PULADO conforme especificação)
    _log("AVISO: Instalacao silenciosa via download direto nao implementada para RustDesk — usar Chocolatey/WinGet", "AVISO")

    _log("✗ Falha absoluta ao instalar RustDesk após 3 tentativas", "ERRO")
    return False
