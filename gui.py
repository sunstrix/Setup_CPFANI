"""gui.py — V5.9.6 (Edição CP Fani: Interface Estabilizada, Remoção Kudu e Centralização OAuth2)"""
...
def load_settings():
    ...
    default_settings = {
        "apps": {
            "choco": ["googlechrome", "anydesk", "flameshot", "vlc", "winrar", "ditto", "vcredist-all", "rustdesk"]
        },
...
# ============================================================
# ITEM 2: Centralização da lógica OAuth2/Google Drive
# ============================================================
def _get_google_drive_service_and_snapshot_files():
    """
    Retorna (service, files) já autenticado, ou (None, []) em caso de falha.
    Centraliza a lógica de autenticação OAuth2 e a busca dos arquivos
    CPFANI_Hardware_Snapshot_* na pasta do Google Drive.
    """
    try:
...
def _read_monitors_from_hardware_snapshots():
    """Lê arquivos CPFANI_Hardware_Snapshot*.txt do Google Drive e extrai dados de monitores."""
    monitors_data = []
...
def _create_inventory_spreadsheet_with_monitors():
    """Cria/atualiza a planilha de inventário GB com a aba 'Periféricos - Monitores'"""
    try:
        import openpyxl
        ...
        for item in monitors_data:
            ws.append([item['Nome_PC'], item['Data_Snapshot'], item['Numero_Monitor'], item['Modelo'], item['Serial']])
...
# ============================================================
# NOVAS FUNÇÕES: INVENTÁRIO GB — IMPRESSORAS
# ============================================================
...
def _read_printers_from_hardware_snapshots():
    """Lê arquivos CPFANI_Hardware_Snapshot*.txt do Google Drive e extrai dados de impressoras."""
    printers_data = []
    service, files = _get_google_drive_service_and_snapshot_files()
    if service is None:
...
def _create_inventory_spreadsheet_with_printers():
    """Cria/atualiza a planilha de inventário GB com a aba 'Periféricos - Impressoras'"""
    try:
        import openpyxl
        from openpyxl.styles import Font, PatternFill, Alignment
        ...
        for item in printers_data:
            ws.append([item['Nome_PC'], item['Data_Snapshot'], item['Nome_Impressora'], item['Porta'], item['Driver'], item['IP'], item['Modelo_SNMP'], item['Serial_SNMP']])
...
class CPFani_GUI(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Setup Automatizado CP Fani — V5.9.6")
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header_frame, text="v5.9.6  |  Gestão de Endpoints (Adaptação Dinâmica)", font=("Segoe UI", 11), text_color="#666666").pack()
        ...
        # ITEM 8: Bloco Kudu removido integralmente
        ...
        self.btn_run = ctk.CTkButton(
            self.main_scroll, text="▶ EXECUTAR DEPLOY", font=("", 14, "bold"), height=40, fg_color="#3a86ff", hover_color="#2a76ef", command=self.start_deploy
        )
        self.btn_run.pack(pady=(10, 5), padx=20, fill="x")
        
        self.btn_snapshot = ctk.CTkButton(
            self.main_scroll, text="📸 GERAR APENAS SNAPSHOT", font=("", 13, "bold"), height=38, fg_color="#2b8a3e", hover_color="#1b7a2e", command=self.start_snapshot_only
        )
    ...
    # ITEM 8: select_all_kudu_actions removido integralmente
    ...
    def _coletar_dados_snapshot(self):
        ...
        locais = [
            "BPCS – LOJA", "4830 – MATRIZ", "4842 – METRÓPOLE", "5152 – CORAÇÃO", "6105 – ASSAI",
            "6106 – DIREITA", "6110 – AROUCHE", "8001 – DOM JOSÉ", "12055 – SÃO BENTO", "11576 – D'AVÓ",
            "12605 – COOP", "12645 – LIGHT", "20371 – METRÔ LUZ", "21502 – BB_SBC", "23000 – OUTLET",
            "12056 – MARECHAL", "14120 – ARPEL SBC", "14353 – ARPEL SP", "23379 – Piraporinha"
        ]
    ...
    def _work(self):
        ...
        try:
            ...
            # Cálculo de tarefas totais (Kudu removido, Apps Legados adicionado)
            total_tasks = 5  # Interface, Segurança, Apps Legados, Agendamentos, Startup
            ...
            # 3. REMOÇÃO DE APPS LEGADOS (ITEM 9)
            ...
            try:
                ...
                for app, removido in resultado_remocao.items():
                    if removido:
                        self.log(f"✓ {app} removido com sucesso.", "OK")
                    else:
                        self.log(f"{app} não encontrado ou já removido.", "INFO")
            except Exception as e:
    ...
    def _finalizar(self, erros):
        ...
        try:
            ...
            if messagebox.askyesno("Reiniciar Computador", "O provisionamento do computador foi concluído com sucesso.\n\nDeseja reiniciar o computador agora para aplicar todas as diretivas de teclado de forma definitiva?"):
                ...
                subprocess.Popen(["shutdown", "/r", "/t", "5", "/f"], creationflags=0x08000000, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception as e:
