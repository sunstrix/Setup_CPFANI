...
def _notify_google_drive_status(message):
    ...
    cb = _GOOGLE_DRIVE_STATUS_CALLBACK
    if cb:
        try:
            cb(message)
        except Exception as e:
...
def _run_oauth_flow_with_timeout(flow, timeout_seconds=120):
    ...
    _notify_google_drive_status("Aguardando login no navegador... Verifique se uma janela foi aberta.")

    try:
        sig = inspect.signature(flow.run_local_server)
        if "timeout" in sig.parameters:
            try:
                return flow.run_local_server(port=0, timeout=timeout_seconds)
            except TypeError:
                pass
    except Exception:
        pass
...
def _authenticate_google_drive_local(timeout_seconds=120):
    ...
    try:
        ...
        _notify_google_drive_status("Verificando credenciais Google Drive...")
        print(f"[INFO] Drive auth: credenciais em {credentials_path}", flush=True)
        print(f"[INFO] Drive auth: token em {token_path}", flush=True)

        if not os.path.exists(credentials_path):
            msg = "Arquivo de credenciais OAuth2 nao encontrado. Upload para Drive desabilitado."
        ...
        if os.path.exists(token_path):
            try:
                ...
                if not isinstance(loaded, Credentials):
                    _notify_google_drive_status("token.pickle corrompido (tipo invalido). Arquivo sera removido.")
        ...
        if creds is not None:
            ...
            if current_scopes and not required_scopes.issubset(current_scopes):
                _notify_google_drive_status("Token atual nao possui escopo completo de Drive. Reautenticando...")
                creds = None

        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                try:
                    _notify_google_drive_status("Renovando token OAuth2...")
                    creds.refresh(Request())
                    _notify_google_drive_status("[OK] Token OAuth2 renovado com sucesso.")
                except Exception as e:
                    _notify_google_drive_status(f"[AVISO] Falha ao renovar token: {e}. Reautenticacao via navegador sera solicitada.")
                    creds = None

            if not creds or not creds.valid:
                flow = InstalledAppFlow.from_client_secrets_file(credentials_path, SCOPES)
                creds = _run_oauth_flow_with_timeout(flow, timeout_seconds)
                _notify_google_drive_status("[OK] Autenticacao OAuth2 concluida.")

                try:
                    with open(token_path, "wb") as token:
                        pickle.dump(creds, token)
                    _notify_google_drive_status(f"[OK] token.pickle salvo em: {token_path}")
                except Exception as e:
                    _notify_google_drive_status(f"[AVISO] Falha ao salvar token.pickle: {e}")

        service = build("drive", "v3", credentials=creds)
        email = _get_drive_user_email(service)

        if email:
            _notify_google_drive_status(f"✓ Conta Google conectada: {email}")
        else:
            _notify_google_drive_status("✓ Conta Google conectada.")
        ...
        msg = f"Bibliotecas do Google Drive nao instaladas: {e}"
        print(f"[ERRO] {msg}", flush=True)
        print("Instale com: python -m pip install google-api-python-client google-auth-oauthlib google-auth-httplib2", flush=True)
        return None, msg, None
    except Exception as e:
        msg = f"Erro inesperado na autenticacao Google Drive: {e}"
        print(f"[ERRO] {msg}", flush=True)
        traceback.print_exc()
        return None, msg, None
...
def _authenticate_google_drive_safe(timeout_seconds=120):
    ...
    if hasattr(mod_config, "authenticate_google_drive"):
        try:
            try:
                result = mod_config.authenticate_google_drive(
                    timeout_seconds=timeout_seconds,
                    callback=_notify_google_drive_status
                )
            except TypeError:
                result = mod_config.authenticate_google_drive()
...
def _get_google_drive_service_and_snapshot_files(timeout_seconds=120):
    ...
    try:
        ...
        service, auth_error, email = _authenticate_google_drive_safe(timeout_seconds)

        if service is None:
            print(f"[AVISO] Google Drive indisponivel: {auth_error}", flush=True)
            return None, []
...
class CPFani_GUI(ctk.CTk):
    def __init__(self):
        ...
        self.snapshot_local_path = None
        self.snapshot_upload_success = True
        self.snapshot_upload_error = None
        self.drive_upload_disabled = False

        global _GOOGLE_DRIVE_STATUS_CALLBACK
        _GOOGLE_DRIVE_STATUS_CALLBACK = self._google_drive_status_background
    ...
    def _google_drive_status_background(self, message):
        """Callback seguro para receber status do Drive a partir de threads."""
        try:
            self.after(0, self._google_drive_status_ui, str(message))
        except Exception as e:
            print(f"[AVISO] Falha ao agendar status do Drive: {e}", flush=True)

    def _google_drive_status_ui(self, message):
        """Atualiza status/log na GUI durante autenticacao do Drive."""
        try:
            self.status_label.configure(text=message)
            self._log_safe(f"[{datetime.now().strftime('%H:%M:%S')}] [DRIVE] {message}\n")
            self.update_idletasks()
        except Exception as e:
            print(f"[AVISO] Falha ao atualizar status do Drive: {e}", flush=True)
    ...
    def _build_ui(self):
        ...
        self.drive_precheck = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(tasks_frame, text="Verificar Google Drive antes de iniciar", variable=self.drive_precheck).pack(anchor="w", padx=10, pady=2)
    ...
    def _interpret_snapshot_result(self, raw_result):
        """Interpreta retorno antigo/novo do snapshot sem quebrar compatibilidade."""
        path = None
        success = True
        error = None
        ...
        try:
            ...
            if path is None:
                success = False
                if not error:
                    error = "Falha ao gerar snapshot local."
    ...
    def _call_snapshot_function(self, func, *args, **kwargs):
        """Chama funcao de snapshot injetando callback do Drive ou bloqueando upload quando solicitado."""
        if hasattr(mod_config, "authenticate_google_drive"):
            original_auth = mod_config.authenticate_google_drive
            ...
            try:
                mod_config.authenticate_google_drive = replacement
                return func(*args, **kwargs)
            finally:
                mod_config.authenticate_google_drive = original_auth
    ...
    def _precheck_drive_then_deploy(self):
        ...
        try:
            self.update_status("Pre-checagem Google Drive...", 0, "Verificando Google Drive...")
            self.log("Pre-checagem Google Drive ativada pelo usuario.", "INFO")

            drive_ok = False
            drive_error = "Falha desconhecida"
            email = None

            if hasattr(mod_config, "pre_check_google_drive_auth"):
                try:
                    drive_ok, drive_error, email = mod_config.pre_check_google_drive_auth(
                        timeout_seconds=120,
                        callback=self._google_drive_status_background
                    )
                except TypeError:
                    drive_ok, drive_error, email = mod_config.pre_check_google_drive_auth()
            else:
                service, files = _get_google_drive_service_and_snapshot_files(timeout_seconds=120)
                drive_ok = service is not None
                if not drive_ok:
                    drive_error = "Falha ao autenticar ou acessar a pasta do Google Drive."

            if drive_ok:
                msg = f"✓ Conta Google conectada: {email}" if email else "✓ Conta Google conectada."
                self.log(msg, "OK")
                self._google_drive_status_background(msg)
                self.after(0, self._start_deploy_worker)
            else:
                self.after(0, self._ask_continue_without_drive, drive_error or "Falha desconhecida")

        except Exception as e:
            self.after(0, self._ask_continue_without_drive, str(e))

    def _ask_continue_without_drive(self, reason):
        """Pergunta se o deploy deve continuar sem upload quando a pre-checagem falha."""
        reason = str(reason or "Falha desconhecida")
        self.log(f"Falha na pre-checagem do Google Drive: {reason}", "AVISO")

        if messagebox.askyesno(
            "Google Drive indisponivel",
            "Falha ao verificar o Google Drive antes do deploy.\n\n"
            f"Motivo: {reason}\n\n"
            "Deseja continuar o deploy sem envio ao Drive?\n"
            "O snapshot sera gerado apenas localmente."
        ):
            self.drive_upload_disabled = True
            self.snapshot_upload_success = False
            self.snapshot_upload_error = f"Pre-checagem falhou e upload foi desabilitado pelo tecnico. Motivo: {reason}"
            self._start_deploy_worker()
        else:
            self.log("Deploy cancelado pelo usuario apos falha na pre-checagem.", "AVISO")
            self.btn_run.configure(state="normal", text="EXECUTAR DEPLOY")
            self.btn_snapshot.configure(state="normal")
            self.update_status("Deploy cancelado.", 0, "")

    def _start_deploy_worker(self):
        """Inicia a thread principal do deploy."""
    ...
    def start_deploy(self):
        ...
        if self.drive_precheck.get():
            thread = threading.Thread(target=self._precheck_drive_then_deploy, daemon=True)
            thread.start()
        else:
            thread = threading.Thread(target=self._safe_work, daemon=True)
            thread.start()
    ...
    def _snapshot_only_work(self):
        ...
        try:
            ...
            try:
                ...
                snapshot_path, upload_success, upload_error = self._interpret_snapshot_result(raw_snapshot)

                self.snapshot_local_path = snapshot_path
                self.snapshot_upload_success = upload_success
                self.snapshot_upload_error = upload_error
    ...
    def _finalizar_snapshot_only(self, erros):
        ...
        try:
            ...
            elif getattr(self, "snapshot_upload_success", True) is False:
                self.update_status("Snapshot local gerado, sem envio ao Drive", 100)
                self.log("[AVISO] Snapshot gerado localmente, mas sem envio ao Drive.", "AVISO")
                show_windows_toast("Aviso no Snapshot", "Snapshot gerado localmente sem envio ao Drive.")
            else:
            ...
            self._alert_snapshot_upload_if_needed()
            ...
            try:
                ...
                if snapshot_path:
                    ...
                    if upload_success:
                        ...
                        self.log(f"[AVISO] Snapshot nao enviado ao Drive. Motivo: {upload_error}", "AVISO")
                else:
                    self.log("Falha ao gerar snapshot de hardware", "ERRO")
    ...
    def _finalizar(self, erros):
        ...
        try:
            ...
            if erros:
                ...
                self.update_status("Deploy concluido sem envio do snapshot ao Drive", 100)
                self.log("[AVISO] Deploy concluido, mas o snapshot nao foi enviado ao Drive.", "AVISO")
                show_windows_toast("Aviso no Provisionamento", "Snapshot nao enviado ao Drive.")
            else:
